...
$(BUILDDIR)/enwiktionary_translations $(BUILDDIR)/enwiktionary_templates $(BUILDDIR)/spanish_tools:
>   git clone -q https://github.com/doozan/$(@F) $@

$(BUILDDIR)/enwiktionary_wordlist: $(BUILDDIR)/enwiktionary_templates $(BUILDDIR)/spanish_tools
>   git clone -q https://github.com/doozan/$(@F) $@
...
$(BUILDDIR)/es-en.enwikt.txt.bz2 $(BUILDDIR)/pt-en.enwikt.txt.bz2 $(BUILDDIR)/fr-en.enwikt.txt.bz2 &: $(BUILDDIR)/enwiktionary-$(DATETAG)-pages-articles.xml.bz2
>   @echo "Making $@..."
>   $(MAKE_EXTRACT) --xml $< --lang pt --lang fr --lang es --outdir $(BUILDDIR)
...
>   @echo "Making $@..."
>   $(MAKE_ALLFORMS) --resolve-lemmas $< > $@


# Sentences

$(BUILDDIR)/%_sentences_detailed.tsv.bz2 $(BUILDDIR)/%-spa_links.tsv.bz2:
>   @echo "Making $@..."
...
$(BUILDDIR)/%_sentences.tsv: $(BUILDDIR)/eng-spa.tsv $(BUILDDIR)/spa-only.txt.json $(BUILDDIR)/%.data-full $(BUILDDIR)/%.allforms.csv
>   @echo "Making $@..."
...
$(BUILDDIR)/%.frequency.csv: $(BUILDDIR)/probabilitats.dat  $(BUILDDIR)/%.data-full $(BUILDDIR)/%.allforms.csv $(BUILDDIR)/es.wordcount sentences.tsv
>   echo "Making $@..."
>   $(MAKE_FREQ) \
...
sentences.csv: $(BUILDDIR)/es-en.enwikt.sentences.csv
>   @echo "Making $@..."
>   cp $< $@
...
%.ifo %.slob: $(BUILDDIR)/%.dictunformat
>   @echo "Making $@..."
>   rm -rf $@
>   $(PYGLOSSARY) --ui=none $< $@

tools: $(BUILDDIR)/enwiktionary_wordlist $(BUILDDIR)/enwiktionary_templates $(BUILDDIR)/enwiktionary_parser $(BUILDDIR)/enwiktionary_translations $(BUILDDIR)/spanish_tools $(BUILDDIR)/spanish_custom
all: tools es-en.data es_allforms.csv sentences.tsv frequency.csv es_merged_50k.txt es-en.enwikt.slob en-es.enwikt.slob es-en.enwikt.ifo
